...
class BatchPublicationReceipt:
    ...
    def __post_init__(self) -> None:
        _require_text("publication repository", self.repository)
        _require_object_id("publication batch_sha", self.batch_sha)
        _require_text("publication branch_ref", self.branch_ref)
        _require_digest(
            "publication evidence_manifest_digest", self.evidence_manifest_digest
        )
        _require_text("publication source_ref", self.source_ref)
        _require_digest("publication receipt_digest", self.receipt_digest)
        if self.receipt_digest != digest_value(
            {"kind": "batch-publication.v1", **self.body()}
        ):
            raise _batch_error(
                "BATCH_PUBLICATION_RECEIPT_MISMATCH",
                "publication receipt digest changed after readback",
            )
...
class PullRequestReadback:
    ...
    def __post_init__(self) -> None:
        if type(self.number) is not int or self.number <= 0:
            raise _batch_error("BATCH_PULL_REQUEST_INVALID", "PR number is invalid")
        _require_text("pull request repository", self.repository)
        _require_object_id("pull request head_sha", self.head_sha)
        _require_text("pull request base_branch", self.base_branch)
        if self.merge_commit_sha is not None:
            _require_object_id(
                "pull request merge_commit_sha", self.merge_commit_sha
            )
        if self.merge_method not in {"merge", "squash", "rebase", "unknown"}:
            raise _batch_error(
                "BATCH_PULL_REQUEST_INVALID", "PR merge method is not closed"
            )
        _require_text("pull request source_ref", self.source_ref)
        _require_digest("pull request readback_digest", self.readback_digest)
        if self.readback_digest != digest_value(
            {"kind": "pull-request-readback.v1", **self.body()}
        ):
            raise _batch_error(
                "BATCH_PULL_REQUEST_MISMATCH",
                "pull request readback digest changed after observation",
            )
...
class HostedResultObservation:
    ...
    def __post_init__(self) -> None:
        _require_text("hosted repository", self.repository)
        _require_object_id("hosted batch_sha", self.batch_sha)
        _require_text("hosted suite_id", self.suite_id)
        _require_text("hosted provider_check_id", self.provider_check_id)
        if self.outcome not in {
            "pending",
            "passed",
            "infrastructure_failure",
            ...
            raise _batch_error(
                "BATCH_HOSTED_OUTCOME_INVALID", "hosted outcome is not closed"
            )
        _require_digest("hosted observation_digest", self.observation_digest)
        _require_text("hosted source_ref", self.source_ref)
...
class TargetIntegrationReadback:
    ...
    def __post_init__(self) -> None:
        _require_text("target readback repository", self.repository)
        _require_text("target readback branch", self.target_branch)
        _require_object_id("target readback target_head_sha", self.target_head_sha)
        _require_object_id("target readback batch_sha", self.batch_sha)
        if type(self.pull_request_number) is not int or self.pull_request_number <= 0:
            raise _batch_error(
                "BATCH_TARGET_READBACK_INVALID", "target PR number is invalid"
            )
        _require_object_id(
            "target readback pull_request_head_sha", self.pull_request_head_sha
        )
        _require_object_id("target readback merge_commit_sha", self.merge_commit_sha)
        if self.merge_method not in {"merge", "squash", "rebase", "unknown"}:
            raise _batch_error(
                "BATCH_TARGET_READBACK_INVALID",
                "target merge method is not closed",
            )
        if type(self.batch_is_ancestor) is not bool:
            raise _batch_error(
                "BATCH_TARGET_READBACK_INVALID",
                "target ancestry is not boolean",
            )
        _require_text("target readback source_ref", self.source_ref)
        _require_digest("target readback readback_digest", self.readback_digest)
        if self.readback_digest != digest_value(
            {"kind": "target-readback.v1", **self.body()}
        ):
            raise _batch_error(
                "BATCH_TARGET_READBACK_MISMATCH",
                "target readback digest changed after observation",
            )
    ...
    def body(self) -> dict[str, object]:
        return {
            ...
            "merge_commit_sha": self.merge_commit_sha,
            "merge_method": self.merge_method,
            "batch_is_ancestor": self.batch_is_ancestor,
            "source_ref": self.source_ref,
        }
