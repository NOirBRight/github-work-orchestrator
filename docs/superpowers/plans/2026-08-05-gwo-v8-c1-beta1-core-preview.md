...
$state = [ordered]@{
    ...
    policy_readbacks = [ordered]@{ initial = $null; pr_create_current = @(); pr_ready_current = @(); pr_merge_current = @() }
    ...
    pr = [ordered]@{ created_by_plan = $null; ready_at = $null; ready_approval_sha256 = $null; ready_lease_sha256 = $null; create_authorization_sha256 = $null; ready_authorization_sha256 = $null; create_remote_refs_sha256 = $null; ready_remote_refs_sha256 = $null; current_policy_receipt_sha256 = $null; ready_current_policy_receipt_sha256 = $null; merge_current_policy_receipt_sha256 = $null; merge = $null }
    tracker = [ordered]@{}
...
$origin = (git -C $root remote get-url origin).Trim(); $exit = $LASTEXITCODE; if ($exit -ne 0 -or $origin -notmatch '^https://github\.com/NOirBRight/github-work-orchestrator(?:\.git)?$') { throw 'ORIGIN_REPOSITORY_INVALID' }
$repo = $state.repository; $repoInfo = @(gh api "repos/$repo" 2>&1); $exit = $LASTEXITCODE; if ($exit -ne 0) { throw 'REPOSITORY_READ_FAILED' }; $repoObject = ($repoInfo -join [Environment]::NewLine) | ConvertFrom-Json; if ($repoObject.full_name -ne $repo -or $repoObject.default_branch -ne 'main') { throw 'DEFAULT_BRANCH_INVALID' }
...
$remoteRepositoryRaw = @(gh api "repos/$repo" 2>&1)
$exit = $LASTEXITCODE
if ($exit -ne 0) { throw 'REMOTE_REPOSITORY_READ_FAILED' }
$remoteRepository = ($remoteRepositoryRaw -join [Environment]::NewLine) | ConvertFrom-Json
if ($remoteRepository.full_name -ne $repo -or $remoteRepository.default_branch -ne 'main') { throw 'REMOTE_DEFAULT_BRANCH_INVALID' }
$branch = (git symbolic-ref --quiet --short HEAD).Trim(); $exit = $LASTEXITCODE; if ($exit -ne 0 -or $branch -ne $state.coordinator_branch) { throw 'COORDINATOR_BRANCH_INVALID' }
$head = (git rev-parse HEAD).Trim(); $exit = $LASTEXITCODE; if ($exit -ne 0 -or $head -ne $state.coordinator_head) { throw 'COORDINATOR_HEAD_INVALID' }
$origin = (git -C $root remote get-url origin).Trim(); $exit = $LASTEXITCODE; if ($exit -ne 0 -or $origin -notmatch '^https://github\.com/NOirBRight/github-work-orchestrator(?:\.git)?$') { throw 'ORIGIN_REPOSITORY_INVALID' }
...
function Save-CurrentPolicyEvidence([string]$key,[string]$label,[object]$actionsRaw,[object]$workflowsRaw,[object]$rulesetRaw,[object]$repositoryRaw,[object]$actionsObject,[object]$workflowsObject,[object]$rulesetObject,[object]$repositoryObject) {
    $dir = Join-Path $evidence ('policy-current-' + $key + '-' + [guid]::NewGuid().ToString('N')); New-Item -ItemType Directory -Path $dir -ErrorAction Stop | Out-Null
    [IO.File]::WriteAllText((Join-Path $dir 'actions.json'),($actionsRaw -join [Environment]::NewLine),[Text.UTF8Encoding]::new($false)); [IO.File]::WriteAllText((Join-Path $dir 'workflows.json'),($workflowsRaw -join [Environment]::NewLine),[Text.UTF8Encoding]::new($false)); [IO.File]::WriteAllText((Join-Path $dir 'ruleset.json'),($rulesetRaw -join [Environment]::NewLine),[Text.UTF8Encoding]::new($false)); [IO.File]::WriteAllText((Join-Path $dir 'repository.json'),($repositoryRaw -join [Environment]::NewLine),[Text.UTF8Encoding]::new($false))
    $receipt = New-PolicyReceipt $label $dir $actionsObject $workflowsObject $rulesetObject $repositoryObject
    if ($null -eq $state.policy_readbacks.PSObject.Properties[$key]) { $state.policy_readbacks | Add-Member -NotePropertyName $key -NotePropertyValue @() }
    $history = @($state.policy_readbacks.PSObject.Properties[$key].Value); $record = [ordered]@{ effect_key = $key; receipt = $receipt; captured_at = [DateTime]::UtcNow.ToString('o') }; $state.policy_readbacks.$key = @($history) + @($record); Save-State $state
    $check = Get-Content -Raw -LiteralPath $statePath | ConvertFrom-Json; $checkHistory = @($check.policy_readbacks.PSObject.Properties[$key].Value); if ($checkHistory.Count -eq 0 -or $checkHistory[-1].receipt.sha256 -ne $receipt.sha256) { throw 'CURRENT_POLICY_STATE_READBACK_INVALID' }
    return $receipt
}
function Assert-PrCreateCurrentPolicy([string]$failure) {
    ...
    $currentPolicy = Save-CurrentPolicyEvidence 'pr_create_current' 'pr-create-current' $liveActions $liveWorkflows $liveRuleset $liveRepository $a $w $r $repository
    $state.pr.current_policy_receipt_sha256 = $currentPolicy.sha256; Save-State $state
    $state = Get-Content -Raw -LiteralPath $statePath | ConvertFrom-Json; if ($state.pr.current_policy_receipt_sha256 -ne $currentPolicy.sha256) { throw 'CURRENT_POLICY_BINDING_READBACK_INVALID' }
...
$didCreate = $false; $createAuthorization = $null; $policyReceipt = $null; if ($matching.Count -eq 0) {
    ...
    $finalCreateLease = Assert-PrWriterLease 'PR_CREATE_FINAL_IMMEDIATE_LEASE_INVALID'; $finalCreateApprovalHash = (Get-FileHash -LiteralPath $approvalPath -Algorithm SHA256).Hash.ToLowerInvariant(); $finalCreateLeaseHash = (Get-FileHash -LiteralPath $leasePath -Algorithm SHA256).Hash.ToLowerInvariant(); $finalCreateNow = [DateTime]::UtcNow; $finalCreateFrom = [DateTime]::Parse([string]$finalCreateLease.valid_from).ToUniversalTime(); $finalCreateUntil = [DateTime]::Parse([string]$finalCreateLease.valid_until).ToUniversalTime(); if ($finalCreateApprovalHash -ne $approvalHash -or $finalCreateLeaseHash -ne $leaseHash -or $finalCreateNow -lt $finalCreateFrom -or $finalCreateNow -ge $finalCreateUntil) { throw 'PR_CREATE_FINAL_IMMEDIATE_LEASE_INVALID' }
        $null = Assert-PrCreateCurrentPolicy 'PR_CREATE_FINAL_CURRENT_POLICY_INVALID'
$createRemoteRows = @(git -C $root ls-remote --heads origin refs/heads/main refs/heads/codex/gwo-v8-beta1 refs/heads/codex/gwo-v8-ga-plan)
$exit = $LASTEXITCODE
if ($exit -ne 0 -or $createRemoteRows.Count -ne 3) { throw 'PR_CREATE_REMOTE_READ_FAILED' }
$createRemote = [ordered]@{}
foreach ($row in $createRemoteRows) { $parts = $row -split '\s+'; if ($parts.Count -ne 2) { throw 'PR_CREATE_REMOTE_ROW_INVALID' }; $createRemote[$parts[1]] = $parts[0] }
if ($createRemote['refs/heads/main'] -ne $state.identities.base.sha -or $createRemote['refs/heads/codex/gwo-v8-beta1'] -ne $state.identities.beta1.sha -or $createRemote['refs/heads/codex/gwo-v8-ga-plan'] -ne $state.identities.protected_ga.sha) { throw 'PR_CREATE_REMOTE_IDENTITY_INVALID' }
$createRemoteReadback = [ordered]@{ schema = 'gwo-v8-c1-pr-create-remote-readback.v1'; refs = $createRemote }
$createRemotePath = Join-Path $evidence 'pr-create-remote-readback.json'; $createRemoteText = $createRemoteReadback | ConvertTo-Json -Depth 20
if (Test-Path -LiteralPath $createRemotePath -PathType Leaf) { if ((Get-Content -Raw -LiteralPath $createRemotePath) -ne $createRemoteText) { throw 'PR_CREATE_REMOTE_RECEIPT_CONFLICT' } } else { [IO.File]::WriteAllText($createRemotePath,$createRemoteText,[Text.UTF8Encoding]::new($false)) }
$createRemoteHash = (Get-FileHash -LiteralPath $createRemotePath -Algorithm SHA256).Hash.ToLowerInvariant(); $state.pr.create_remote_refs_sha256 = $createRemoteHash; Save-State $state
$state = Get-Content -Raw -LiteralPath $statePath | ConvertFrom-Json; if ($state.pr.create_remote_refs_sha256 -ne $createRemoteHash) { throw 'PR_CREATE_REMOTE_STATE_READBACK_INVALID' }
...
$state.pr = [ordered]@{ number = $prNumber; created_by_plan = [bool]$createdByPlan; head_repository = $pr.head.repo.full_name; base_repository = $pr.base.repo.full_name; head_ref = $pr.head.ref; base_ref = $pr.base.ref; head_sha = $pr.head.sha; paths = @($actualStatus.Keys | Sort-Object); path_status = $actualStatus; draft = $pr.draft; integration_lease_id = $lease.id; approval_sha256 = $approvalHash; lease_sha256 = $leaseHash; ready_at = $null; ready_approval_sha256 = $null; ready_lease_sha256 = $null; create_authorization_sha256 = if ($null -eq $createAuthorization) { $null } else { $createAuthorization.reference.authorization_sha256 }; ready_authorization_sha256 = $null; create_remote_refs_sha256 = $state.pr.create_remote_refs_sha256; ready_remote_refs_sha256 = $state.pr.ready_remote_refs_sha256; current_policy_receipt_sha256 = $state.pr.current_policy_receipt_sha256; ready_current_policy_receipt_sha256 = $state.pr.ready_current_policy_receipt_sha256; merge_current_policy_receipt_sha256 = $state.pr.merge_current_policy_receipt_sha256; merge = $null }; if ($didCreate) { $state.mutation_policy.pr_create = $policyReceipt }; if ($didCreate -and $null -eq $state.mutation_authorization.pr_create) { throw 'PR_CREATE_AUTHORIZATION_STATE_MISSING' }
Save-State $state
~~~
...
if ($didReady) {
    ...
    $finalReadyLease = Assert-PrReadyLease 'PR_READY_FINAL_IMMEDIATE_LEASE_INVALID'; $finalReadyApprovalHash = (Get-FileHash -LiteralPath $approvalPath -Algorithm SHA256).Hash.ToLowerInvariant(); $finalReadyLeaseHash = (Get-FileHash -LiteralPath $leasePath -Algorithm SHA256).Hash.ToLowerInvariant(); $finalReadyNow = [DateTime]::UtcNow; $finalReadyFrom = [DateTime]::Parse([string]$finalReadyLease.valid_from).ToUniversalTime(); $finalReadyUntil = [DateTime]::Parse([string]$finalReadyLease.valid_until).ToUniversalTime(); if ($finalReadyApprovalHash -ne $approvalHash -or $finalReadyLeaseHash -ne $leaseHash -or $finalReadyNow -lt $finalReadyFrom -or $finalReadyNow -ge $finalReadyUntil) { throw 'PR_READY_FINAL_IMMEDIATE_LEASE_INVALID' };     $null = Assert-PrReadyCurrentPolicy 'PR_READY_FINAL_CURRENT_POLICY_INVALID'
$readyRemoteRows = @(git -C $root ls-remote --heads origin refs/heads/main refs/heads/codex/gwo-v8-beta1 refs/heads/codex/gwo-v8-ga-plan)
$exit = $LASTEXITCODE
if ($exit -ne 0 -or $readyRemoteRows.Count -ne 3) { throw 'PR_READY_REMOTE_READ_FAILED' }
$readyRemote = [ordered]@{}
foreach ($row in $readyRemoteRows) { $parts = $row -split '\s+'; if ($parts.Count -ne 2) { throw 'PR_READY_REMOTE_ROW_INVALID' }; $readyRemote[$parts[1]] = $parts[0] }
if ($readyRemote['refs/heads/main'] -ne $state.identities.base.sha -or $readyRemote['refs/heads/codex/gwo-v8-beta1'] -ne $state.identities.beta1.sha -or $readyRemote['refs/heads/codex/gwo-v8-ga-plan'] -ne $state.identities.protected_ga.sha) { throw 'PR_READY_REMOTE_IDENTITY_INVALID' }
$readyRemoteReadback = [ordered]@{ schema = 'gwo-v8-c1-pr-ready-remote-readback.v1'; refs = $readyRemote }
$readyRemotePath = Join-Path $evidence 'pr-ready-remote-readback.json'; $readyRemoteText = $readyRemoteReadback | ConvertTo-Json -Depth 20
if (Test-Path -LiteralPath $readyRemotePath -PathType Leaf) { if ((Get-Content -Raw -LiteralPath $readyRemotePath) -ne $readyRemoteText) { throw 'PR_READY_REMOTE_RECEIPT_CONFLICT' } } else { [IO.File]::WriteAllText($readyRemotePath,$readyRemoteText,[Text.UTF8Encoding]::new($false)) }
$readyRemoteHash = (Get-FileHash -LiteralPath $readyRemotePath -Algorithm SHA256).Hash.ToLowerInvariant(); $state.pr.ready_remote_refs_sha256 = $readyRemoteHash; Save-State $state
$state = Get-Content -Raw -LiteralPath $statePath | ConvertFrom-Json; if ($state.pr.ready_remote_refs_sha256 -ne $readyRemoteHash) { throw 'PR_READY_REMOTE_STATE_READBACK_INVALID' }
...
function Assert-MergeCurrentPolicy([string]$failure) {
    ...
    if ($repository.full_name -ne $repo -or $repository.default_branch -ne 'main' -or $a.enabled -ne $false -or $w.total_count -ne 0 -or $r.id -ne 20160628 -or $r.enforcement -ne 'active' -or $r.source -ne $repo -or @($r.bypass_actors).Count -ne 0 -or @($r.rules | Where-Object type -eq 'required_status_checks').Count -ne 0 -or @(Compare-Object (@('deletion','non_fast_forward','pull_request','required_linear_history') | Sort-Object) ($types | Sort-Object)).Count -ne 0 -or $allowed -notcontains 'squash' -or (-not ($includes -contains '~DEFAULT_BRANCH' -or $includes -contains 'refs/heads/main' -or $includes -contains 'main'))) { throw $failure }
    $currentPolicy = Save-CurrentPolicyEvidence 'pr_merge_current' 'pr-merge-current' $liveActions $liveWorkflows $liveRuleset $liveRepository $a $w $r $repository
    $state.pr.merge_current_policy_receipt_sha256 = $currentPolicy.sha256; Save-State $state
    $state = Get-Content -Raw -LiteralPath $statePath | ConvertFrom-Json; if ($state.pr.merge_current_policy_receipt_sha256 -ne $currentPolicy.sha256) { throw 'CURRENT_POLICY_BINDING_READBACK_INVALID' }
    return $currentPolicy
}
...
if (-not $alreadyMerged) {
    ...
    $mergeFinalApproval = Get-Content -Raw -LiteralPath $approvalPath | ConvertFrom-Json; $mergeFinalLease = Get-Content -Raw -LiteralPath $leasePath | ConvertFrom-Json; $mergeFinalApprovalHash = (Get-FileHash -LiteralPath $approvalPath -Algorithm SHA256).Hash.ToLowerInvariant(); $mergeFinalLeaseHash = (Get-FileHash -LiteralPath $leasePath -Algorithm SHA256).Hash.ToLowerInvariant(); $mergeFinalFrom = [DateTime]::Parse([string]$mergeFinalLease.valid_from).ToUniversalTime(); $mergeFinalUntil = [DateTime]::Parse([string]$mergeFinalLease.valid_until).ToUniversalTime(); $mergeFinalNow = [DateTime]::UtcNow; if ($mergeFinalApprovalHash -ne $approvalHash -or $mergeFinalLeaseHash -ne $leaseHash -or $mergeFinalLease.id -ne $mergeFinalApproval.integration_lease_id -or $mergeFinalLease.approval_sha256 -ne $mergeFinalApprovalHash -or $mergeFinalNow -lt $mergeFinalFrom -or $mergeFinalNow -ge $mergeFinalUntil) { throw 'MERGE_FINAL_IMMEDIATE_LEASE_INVALID' }
        $null = Assert-MergeCurrentPolicy 'MERGE_FINAL_CURRENT_POLICY_INVALID'
