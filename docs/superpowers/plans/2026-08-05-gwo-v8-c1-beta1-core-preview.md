...
function Assert-PrCreatePolicyAttempt([object]$receipt) {
    ...
    $directory = Split-Path -LiteralPath $receipt.path -Parent; $values = [ordered]@{}
...
$didCreate = $false; $createAuthorization = $null; $policyReceipt = $null; if ($matching.Count -eq 0) {
    $orphanDirectory = Join-Path $evidence 'authorizations'; $orphanPaths = @(); if (Test-Path -LiteralPath $orphanDirectory -PathType Container) { $orphanPaths = @(Get-ChildItem -LiteralPath $orphanDirectory -Filter 'pr-create-*.json' -File | Sort-Object FullName | Where-Object { $null -eq $state.mutation_authorization.pr_create -or $_.FullName.Replace('\','/') -ne $state.mutation_authorization.pr_create.authorization_path }) }; if ($orphanPaths.Count -gt 1) { throw 'PR_CREATE_AUTHORIZATION_ORPHAN_CONFLICT' }
    ...
    if ($null -ne $createAuthorization) {
        ...
        $createAuthorization = Read-PrCreateAuthorization $true $orphanPaths[0].FullName; $policyReceipt = Assert-PrCreatePolicyAttempt $createAuthorization.policy; $null = Assert-PrCreateCurrentPolicy 'PR_CREATE_CURRENT_POLICY_INVALID'; $state.mutation_policy.pr_create = $policyReceipt; $state.mutation_authorization.pr_create = $createAuthorization.reference; $state.pr.created_by_plan = $true; Save-State $state; $state = Get-Content -Raw -LiteralPath $statePath | ConvertFrom-Json; $createAuthorization = Read-PrCreateAuthorization $true; Write-Output 'ADOPTED_PR_CREATE_ORPHAN_ATTEMPT'
    ...
    if ($null -eq $leaseEvidence) { $leaseEvidence = Assert-PrWriterLease 'PR_CREATE_LEASE_AUTHORIZATION_INVALID' }; $createAuthorization = Save-PrCreateAuthorization $policyReceipt $leaseEvidence
...
if ($didReady) {
    ...
    if ($null -ne $readyAuthorization) { if ($null -eq $state.mutation_policy.pr_ready) { $policyReceipt = Assert-PrReadyPolicyAttempt $readyAuthorization.policy; $state.mutation_policy['pr_ready'] = $policyReceipt; Save-State $state } else { $policyReceipt = Assert-PrReadyPolicyAttempt $state.mutation_policy.pr_ready; if ($policyReceipt.path -ne $readyAuthorization.policy.path -or $policyReceipt.sha256 -ne $readyAuthorization.policy.sha256) { throw 'PR_READY_POLICY_AUTHORIZATION_MISMATCH' } }; $null = Assert-PrReadyCurrentPolicy 'PR_READY_CURRENT_POLICY_INVALID'; $leaseEvidence = Assert-PrReadyLease 'PR_READY_RESUME_LEASE_INVALID'; Write-Output 'REUSE_RECOVERED_PR_READY_ATTEMPT'
    } elseif ($orphanPaths.Count -eq 1) { $readyAuthorization = Read-PrReadyAuthorization $true $orphanPaths[0].FullName; $policyReceipt = Assert-PrReadyPolicyAttempt $readyAuthorization.policy; $null = Assert-PrReadyCurrentPolicy 'PR_READY_CURRENT_POLICY_INVALID'; $state.mutation_policy['pr_ready'] = $policyReceipt; $state.mutation_authorization.pr_ready = $readyAuthorization.reference; Save-State $state; $state = Get-Content -Raw -LiteralPath $statePath | ConvertFrom-Json; $readyAuthorization = Read-PrReadyAuthorization $true; Write-Output 'ADOPTED_PR_READY_ORPHAN_ATTEMPT'
    ...
    $readyAuthorization = Read-PrReadyAuthorization $true; $policyReceipt = Assert-PrReadyPolicyAttempt $readyAuthorization.policy; if ($null -eq $state.mutation_policy.pr_ready) { $state.mutation_policy['pr_ready'] = $policyReceipt; Save-State $state } elseif ($state.mutation_policy.pr_ready.path -ne $policyReceipt.path -or $state.mutation_policy.pr_ready.sha256 -ne $policyReceipt.sha256) { throw 'PR_READY_EXISTING_EFFECT_POLICY_AUTHORIZATION_MISMATCH' }
}
...
if ($direct.Count -eq 0) {
    ...
    } elseif ($orphanPaths.Count -eq 1) { $tagAuthorization = Read-TagAuthorization $true $orphanPaths[0].FullName; $policyReceipt = Assert-TagPolicyAttempt $tagAuthorization.policy; $null = Assert-CurrentPublicationPolicy 'TAG_CURRENT_POLICY_INVALID'; $state.mutation_policy.tag = $policyReceipt; $state.mutation_authorization.tag = $tagAuthorization.reference; Save-State $state; $state = Get-Content -Raw -LiteralPath $statePath | ConvertFrom-Json; $tagAuthorization = Read-TagAuthorization $true; Write-Output 'ADOPTED_TAG_ORPHAN_ATTEMPT'
