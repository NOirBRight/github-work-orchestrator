...
function Assert-MergePolicyAttempt([object]$receipt) {
    if ($null -eq $receipt -or $receipt.label -ne 'pr-merge' -or [string]::IsNullOrWhiteSpace([string]$receipt.path) -or [string]::IsNullOrWhiteSpace([string]$receipt.sha256) -or -not (Test-Path -LiteralPath $receipt.path -PathType Leaf)) { throw 'MERGE_POLICY_ATTEMPT_INVALID' }
    if ((Get-FileHash -LiteralPath $receipt.path -Algorithm SHA256).Hash.ToLowerInvariant() -ne $receipt.sha256) { throw 'MERGE_POLICY_ATTEMPT_INVALID' }
    try { $value = Get-Content -Raw -LiteralPath $receipt.path | ConvertFrom-Json } catch { throw 'MERGE_POLICY_ATTEMPT_INVALID' }
    if ($value.schema -ne 'gwo-v8-c1-mutation-policy.v1' -or $value.label -ne 'pr-merge' -or $value.repository -ne $repo -or $value.default_branch -ne 'main' -or $value.actions_enabled -ne $false -or $value.workflow_count -ne 0 -or $value.ruleset_id -ne 20160628) { throw 'MERGE_POLICY_ATTEMPT_INVALID' }
    return [ordered]@{ label = 'pr-merge'; path = ([string]$receipt.path).Replace([char]92,'/'); sha256 = [string]$receipt.sha256 }
}
...
function Assert-TrackerPolicyAttempt([object]$receipt,[string]$effectKey) {
    if ($null -eq $receipt -or $receipt.effect_key -ne $effectKey -or [string]::IsNullOrWhiteSpace([string]$receipt.path) -or [string]::IsNullOrWhiteSpace([string]$receipt.sha256) -or -not (Test-Path -LiteralPath $receipt.path -PathType Leaf)) { throw "TRACKER_POLICY_ATTEMPT_INVALID:$effectKey" }
    if ((Get-FileHash -LiteralPath $receipt.path -Algorithm SHA256).Hash.ToLowerInvariant() -ne $receipt.sha256) { throw "TRACKER_POLICY_ATTEMPT_INVALID:$effectKey" }
    try { $value = Get-Content -Raw -LiteralPath $receipt.path | ConvertFrom-Json } catch { throw "TRACKER_POLICY_ATTEMPT_INVALID:$effectKey" }
    if ($value.schema -ne 'gwo-v8-c1-mutation-policy.v1' -or $value.repository -ne $repo -or $value.default_branch -ne 'main' -or $value.actions_enabled -ne $false -or $value.workflow_count -ne 0 -or $value.ruleset_id -ne 20160628) { throw "TRACKER_POLICY_ATTEMPT_INVALID:$effectKey" }
    return [ordered]@{ effect_key = $effectKey; label = [string]$receipt.label; path = ([string]$receipt.path).Replace([char]92,'/'); sha256 = [string]$receipt.sha256 }
}
...
function Assert-ReleasePolicyAttempt([object]$receipt) {
    if ($null -eq $receipt -or $receipt.label -ne 'release-create' -or [string]::IsNullOrWhiteSpace([string]$receipt.path) -or [string]::IsNullOrWhiteSpace([string]$receipt.sha256) -or -not (Test-Path -LiteralPath $receipt.path -PathType Leaf)) { throw 'RELEASE_POLICY_ATTEMPT_INVALID' }
    if ((Get-FileHash -LiteralPath $receipt.path -Algorithm SHA256).Hash.ToLowerInvariant() -ne $receipt.sha256) { throw 'RELEASE_POLICY_ATTEMPT_INVALID' }
    try { $value = Get-Content -Raw -LiteralPath $receipt.path | ConvertFrom-Json } catch { throw 'RELEASE_POLICY_ATTEMPT_INVALID' }
    if ($value.schema -ne 'gwo-v8-c1-mutation-policy.v1' -or $value.label -ne 'release-create' -or $value.repository -ne $repo -or $value.default_branch -ne 'main' -or $value.actions_enabled -ne $false -or $value.workflow_count -ne 0 -or $value.ruleset_id -ne 20160628) { throw 'RELEASE_POLICY_ATTEMPT_INVALID' }
    return [ordered]@{ label = 'release-create'; path = ([string]$receipt.path).Replace([char]92,'/'); sha256 = [string]$receipt.sha256 }
}
