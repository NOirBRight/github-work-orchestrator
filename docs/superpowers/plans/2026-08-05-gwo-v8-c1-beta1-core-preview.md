...
$policyReceipt = New-PolicyReceipt 'pr-create' $policyDir $a $w $r $repoPolicy
...
function Read-PrCreateAuthorization([bool]$required) {
    $reference = $state.mutation_authorization.pr_create; if ($null -eq $reference) { if ($required) { throw 'PR_CREATE_EXISTING_EFFECT_WITHOUT_AUTHORIZATION' }; return $null }
...
function Save-PrCreateAuthorization([object]$policy,[object]$leaseEvidence) {
    ...
    $directory = Join-Path $evidence 'authorizations'; if (-not (Test-Path -LiteralPath $directory -PathType Container)) { New-Item -ItemType Directory -Path $directory -ErrorAction Stop | Out-Null }; $path = Join-Path $directory ("pr-create-$($policy.sha256)-$($leaseEvidence.lease_sha256).json")
    ...
    $saved = Get-Content -Raw -LiteralPath $statePath | ConvertFrom-Json; if ($saved.mutation_authorization.pr_create.authorization_sha256 -ne $reference.authorization_sha256) { throw 'PR_CREATE_AUTHORIZATION_STATE_READBACK_INVALID' }
    return Read-PrCreateAuthorization $true
}
$didCreate = $false; $createAuthorization = $null; if ($matching.Count -eq 0) {
    $rowsAgain = @(gh api "repos/$repo/pulls?state=all&head=NOirBRight:codex/gwo-v8-beta1&base=main&per_page=100" 2>&1); $exit = $LASTEXITCODE; if ($exit -ne 0) { throw 'PR_RACE_READ_FAILED' }; $matchingAgain = @((($rowsAgain -join [Environment]::NewLine) | ConvertFrom-Json) | Where-Object { $_.head.repo.full_name -eq $repo -and $_.base.repo.full_name -eq $repo -and $_.head.ref -eq 'codex/gwo-v8-beta1' -and $_.base.ref -eq 'main' }); if ($matchingAgain.Count -gt 1) { throw 'MULTIPLE_EXACT_PRS' }; if ($matchingAgain.Count -ne 0) { throw 'PR_CREATE_PREMUTATION_CONFLICT' }
    if ($null -ne $state.mutation_policy.pr_create -and ($state.mutation_policy.pr_create.path -ne $policyReceipt.path -or $state.mutation_policy.pr_create.sha256 -ne $policyReceipt.sha256)) { throw 'PR_CREATE_POLICY_RESUME_CONFLICT' }
    $state.mutation_policy.pr_create = $policyReceipt; $state.pr.created_by_plan = $true; Save-State $state; $policyState = Get-Content -Raw -LiteralPath $statePath | ConvertFrom-Json; if ($policyState.mutation_policy.pr_create.sha256 -ne $policyReceipt.sha256 -or $policyState.pr.created_by_plan -ne $true) { throw 'PR_CREATE_INTENT_STATE_READBACK_INVALID' }
    $leaseEvidence = Assert-PrWriterLease 'PR_CREATE_LEASE_AUTHORIZATION_INVALID'; $createAuthorization = Save-PrCreateAuthorization $policyReceipt $leaseEvidence
...
function Read-PrReadyAuthorization([bool]$required) {
    $reference = $state.mutation_authorization.pr_ready; if ($null -eq $reference) { if ($required) { throw 'PR_READY_EXISTING_EFFECT_WITHOUT_AUTHORIZATION' }; return $null }
    if ($reference.effect_key -ne 'pr-ready' -or -not (Test-Path -LiteralPath $reference.authorization_path -PathType Leaf)) { throw 'PR_READY_AUTHORIZATION_REFERENCE_INVALID' }
    $authorizationHash = (Get-FileHash -LiteralPath $reference.authorization_path -Algorithm SHA256).Hash.ToLowerInvariant(); if ($authorizationHash -ne $reference.authorization_sha256) { throw 'PR_READY_AUTHORIZATION_HASH_INVALID' }
    try { $value = Get-Content -Raw -LiteralPath $reference.authorization_path | ConvertFrom-Json; $authorizedAt = [DateTime]::Parse([string]$value.authorized_at).ToUniversalTime(); $authFrom = [DateTime]::Parse([string]$value.valid_from).ToUniversalTime(); $authUntil = [DateTime]::Parse([string]$value.valid_until).ToUniversalTime() } catch { throw 'PR_READY_AUTHORIZATION_MALFORMED' }
    if ($value.schema -ne 'gwo-v8-c1-mutation-authorization.v1' -or $value.operation -ne 'pr-ready' -or $value.effect_key -ne 'pr-ready' -or $value.repository -ne $repo -or [int]$value.pr_number -ne $prNumber -or $value.base_sha -ne $state.identities.base.sha -or $value.head_sha -ne $state.identities.beta1.sha -or $value.owner -ne $approval.owner -or $value.lease_id -ne $lease.id -or $value.approval_sha256 -ne $approvalHash -or $value.lease_sha256 -ne $leaseHash -or $value.policy_label -ne 'pr-ready' -or $authFrom -ne $leaseFrom -or $authUntil -ne $leaseUntil -or $authorizedAt -lt $authFrom -or $authorizedAt -ge $authUntil) { throw 'PR_READY_AUTHORIZATION_INVALID' }
    if (-not (Test-Path -LiteralPath $value.policy_path -PathType Leaf) -or (Get-FileHash -LiteralPath $value.policy_path -Algorithm SHA256).Hash.ToLowerInvariant() -ne $value.policy_sha256) { throw 'PR_READY_AUTHORIZATION_POLICY_INVALID' }
    return [ordered]@{ reference = $reference; value = $value; policy = [ordered]@{ label = 'pr-ready'; path = [string]$value.policy_path; sha256 = [string]$value.policy_sha256 } }
}
function Save-PrReadyAuthorization([object]$policy,[object]$leaseEvidence) {
    ...
    $authorizedAt = [DateTime]::UtcNow; $authFrom = [DateTime]::Parse([string]$leaseEvidence.valid_from).ToUniversalTime(); $authUntil = [DateTime]::Parse([string]$leaseEvidence.valid_until).ToUniversalTime(); if ($authorizedAt -lt $authFrom -or $authorizedAt -ge $authUntil) { throw 'PR_READY_AUTHORIZATION_OUTSIDE_LEASE' }
    $directory = Join-Path $evidence 'authorizations'; if (-not (Test-Path -LiteralPath $directory -PathType Container)) { New-Item -ItemType Directory -Path $directory -ErrorAction Stop | Out-Null }; $path = Join-Path $directory ("pr-ready-$prNumber-$($policy.sha256)-$($leaseEvidence.lease_sha256).json")
    ...
    $json = $value | ConvertTo-Json -Depth 30; if (Test-Path -LiteralPath $path -PathType Leaf) { if ((Get-Content -Raw -LiteralPath $path) -ne $json) { throw 'PR_READY_AUTHORIZATION_CONFLICT' } } else { [IO.File]::WriteAllText($path,$json,[Text.UTF8Encoding]::new($false)) }
    $reference = [ordered]@{ effect_key = 'pr-ready'; authorization_path = $path.Replace('\','/'); authorization_sha256 = (Get-FileHash -LiteralPath $path -Algorithm SHA256).Hash.ToLowerInvariant() }; $state.mutation_authorization.pr_ready = $reference; Save-State $state
    $saved = Get-Content -Raw -LiteralPath $statePath | ConvertFrom-Json; if ($saved.mutation_authorization.pr_ready.authorization_sha256 -ne $reference.authorization_sha256) { throw 'PR_READY_AUTHORIZATION_STATE_READBACK_INVALID' }
...
$repoPolicyRaw = @(gh api repos/$repo 2>&1); $exit = $LASTEXITCODE; if ($exit -ne 0) { throw 'REPOSITORY_POLICY_READ_FAILED' }; $repoPolicy = ($repoPolicyRaw -join [Environment]::NewLine) | ConvertFrom-Json; if ($repoPolicy.full_name -ne $repo -or $repoPolicy.default_branch -ne 'main') { throw 'DEFAULT_BRANCH_POLICY_INVALID' }; [IO.File]::WriteAllText((Join-Path $policyDir 'repository.json'),($repoPolicyRaw -join [Environment]::NewLine),[Text.UTF8Encoding]::new($false))
$policyReceipt = New-PolicyReceipt 'pr-ready' $policyDir $a $w $r $repoPolicy; $didReady = $pr.draft -eq $true; $readyAuthorization = $null
if ($didReady) {
    $state.mutation_policy.pr_ready = $policyReceipt; Save-State $state; $policyState = Get-Content -Raw -LiteralPath $statePath | ConvertFrom-Json; if ($policyState.mutation_policy.pr_ready.sha256 -ne $policyReceipt.sha256) { throw 'PR_READY_POLICY_STATE_READBACK_INVALID' }
    $leaseEvidence = Assert-PrReadyLease 'PR_READY_LEASE_AUTHORIZATION_INVALID'; $readyAuthorization = Save-PrReadyAuthorization $policyReceipt $leaseEvidence
    $finalReadyLease = Assert-PrReadyLease 'PR_READY_FINAL_IMMEDIATE_LEASE_INVALID'; $finalReadyApprovalHash = (Get-FileHash -LiteralPath $approvalPath -Algorithm SHA256).Hash.ToLowerInvariant(); $finalReadyLeaseHash = (Get-FileHash -LiteralPath $leasePath -Algorithm SHA256).Hash.ToLowerInvariant(); $finalReadyNow = [DateTime]::UtcNow; $finalReadyFrom = [DateTime]::Parse([string]$finalReadyLease.valid_from).ToUniversalTime(); $finalReadyUntil = [DateTime]::Parse([string]$finalReadyLease.valid_until).ToUniversalTime(); if ($finalReadyApprovalHash -ne $approvalHash -or $finalReadyLeaseHash -ne $leaseHash -or $finalReadyNow -lt $finalReadyFrom -or $finalReadyNow -ge $finalReadyUntil) { throw 'PR_READY_FINAL_IMMEDIATE_LEASE_INVALID' }
    gh pr ready $prNumber --repo $repo; $exit = $LASTEXITCODE; if ($exit -ne 0) { throw 'PR_READY_FAILED' }
} else {
    $readyAuthorization = Read-PrReadyAuthorization $true; $policyReceipt = $readyAuthorization.policy; if ($null -eq $state.mutation_policy.pr_ready -or $state.mutation_policy.pr_ready.path -ne $policyReceipt.path -or $state.mutation_policy.pr_ready.sha256 -ne $policyReceipt.sha256) { throw 'PR_READY_EXISTING_EFFECT_POLICY_AUTHORIZATION_MISMATCH' }
}
...
if ($savedApproval.approval_sha256 -ne $approvalHash -or $savedApproval.lease_sha256 -ne $leaseHash -or $savedApproval.owner -ne $approval.owner -or $savedApproval.lease_id -ne $lease.id -or $savedApproval.writer_scope -ne 'repository-global-remote-writer' -or $savedApproval.exclusive_remote_writer_control -ne $true -or $savedApproval.base_sha -ne $approval.base_sha -or $savedApproval.base_tree -ne $lease.base_tree -or $savedApproval.head_sha -ne $approval.head_sha -or $savedApproval.head_tree -ne $lease.head_tree -or @(Compare-Object $expectedPrScope $savedScope).Count -ne 0) { throw 'PR_APPROVAL_STATE_BINDING_INVALID' }
function New-PolicyReceipt([string]$label,[string]$directory,[object]$actionsObject,[object]$workflowsObject,[object]$rulesetObject,[object]$repositoryObject) { $files = [ordered]@{}; foreach ($name in @('actions.json','workflows.json','ruleset.json','repository.json')) { $path = Join-Path $directory $name; $files[$name] = [ordered]@{ path = $path.Replace('\','/'); sha256 = (Get-FileHash -LiteralPath $path -Algorithm SHA256).Hash.ToLowerInvariant() } }; $receipt = [ordered]@{ schema = 'gwo-v8-c1-mutation-policy.v1'; label = $label; repository = $repositoryObject.full_name; default_branch = $repositoryObject.default_branch; actions_enabled = $actionsObject.enabled; workflow_count = $workflowsObject.total_count; ruleset_id = $rulesetObject.id; files = $files }; $path = Join-Path $directory 'receipt.json'; $json = $receipt | ConvertTo-Json -Depth 30; if (Test-Path -LiteralPath $path -PathType Leaf) { if ((Get-Content -Raw -LiteralPath $path) -ne $json) { throw 'POLICY_RECEIPT_CONFLICT' } } else { [IO.File]::WriteAllText($path,$json,[Text.UTF8Encoding]::new($false)) }; $policyReceipt = [ordered]@{ label = $label; path = $path.Replace('\','/'); sha256 = (Get-FileHash -LiteralPath $path -Algorithm SHA256).Hash.ToLowerInvariant() }; return $policyReceipt }
function Read-MergeAuthorization {
    $reference = $state.mutation_authorization.pr_merge; if ($null -eq $reference) { return $null }
...
function Save-MergeAuthorization([object]$policy,[DateTime]$from,[DateTime]$until) {
    ...
    $directory = Join-Path $evidence 'authorizations'; if (-not (Test-Path -LiteralPath $directory -PathType Container)) { New-Item -ItemType Directory -Path $directory -ErrorAction Stop | Out-Null }
...
$alreadyMerged = $prBefore.merged -eq $true; $mergeAuthorization = Read-MergeAuthorization; $policyReceipt = $state.mutation_policy.pr_merge
if (-not $alreadyMerged) {
    ...
    $mergeAuthorization = Save-MergeAuthorization $policyReceipt $fromImmediate $untilImmediate
...
function Get-TrackerAuthorization([string]$effectKey,[bool]$required) {
    ...
    if ($matches.Count -eq 0) { if ($required) { throw "TRACKER_EFFECT_WITHOUT_AUTHORIZATION:$effectKey" }; return $null }
    $reference = $matches[0]; if (-not (Test-Path -LiteralPath $reference.authorization_path -PathType Leaf)) { throw "TRACKER_AUTHORIZATION_MISSING:$effectKey" }
...
function Save-TrackerAuthorization([string]$effectKey,[object]$policy,[object]$leaseEvidence) {
    ...
    $directory = Join-Path $evidence 'tracker-authorizations'; if (-not (Test-Path -LiteralPath $directory -PathType Container)) { New-Item -ItemType Directory -Path $directory -ErrorAction Stop | Out-Null }; $safe = $effectKey -replace '[^A-Za-z0-9.-]','-'; $path = Join-Path $directory ("$safe-$($policy.sha256).json")
...
foreach ($title in $map.Keys) {
    ...
    } elseif ($writeRequired) { $authorization = Get-TrackerAuthorization $effectKey $true }
...
foreach ($title in $map.Keys) {
    ...
    foreach ($number in $map[$title]) {
        ...
        } else { $final = $before; if ($writeRequired) { $authorization = Get-TrackerAuthorization $effectKey $true } }
...
} elseif ($reopenRequired -and $issue137.semantic.state -eq 'open') { $after137 = $issue137; $reopenAuthorization = Get-TrackerAuthorization $reopenEffectKey $true
...
function Read-TagAuthorization([bool]$required) {
    $reference = $state.mutation_authorization.tag; if ($null -eq $reference) { if ($required) { throw 'TAG_EXISTING_EFFECT_WITHOUT_AUTHORIZATION' }; return $null }
    if ($reference.effect_key -ne 'tag:v8.0.0-beta.1' -or -not (Test-Path -LiteralPath $reference.authorization_path -PathType Leaf)) { throw 'TAG_AUTHORIZATION_REFERENCE_INVALID' }
    $authorizationHash = (Get-FileHash -LiteralPath $reference.authorization_path -Algorithm SHA256).Hash.ToLowerInvariant(); if ($authorizationHash -ne $reference.authorization_sha256) { throw 'TAG_AUTHORIZATION_HASH_INVALID' }
    try { $value = Get-Content -Raw -LiteralPath $reference.authorization_path | ConvertFrom-Json; $authorizedAt = [DateTime]::Parse([string]$value.authorized_at).ToUniversalTime(); $authFrom = [DateTime]::Parse([string]$value.valid_from).ToUniversalTime(); $authUntil = [DateTime]::Parse([string]$value.valid_until).ToUniversalTime() } catch { throw 'TAG_AUTHORIZATION_MALFORMED' }
    if ($value.schema -ne 'gwo-v8-c1-mutation-authorization.v1' -or $value.operation -ne 'tag-push' -or $value.effect_key -ne 'tag:v8.0.0-beta.1' -or $value.repository -ne $repo -or $value.merged_main_sha -ne $mergedSha -or $value.tag_name -ne $tagName -or $value.owner -ne $approval.owner -or $value.lease_id -ne $lease.id -or $value.approval_sha256 -ne $approvalHash -or $value.lease_sha256 -ne $leaseHash -or $value.policy_label -ne 'tag-push' -or $authFrom -ne $leaseFrom -or $authUntil -ne $leaseUntil -or $authorizedAt -lt $authFrom -or $authorizedAt -ge $authUntil) { throw 'TAG_AUTHORIZATION_INVALID' }
    if (-not (Test-Path -LiteralPath $value.policy_path -PathType Leaf) -or (Get-FileHash -LiteralPath $value.policy_path -Algorithm SHA256).Hash.ToLowerInvariant() -ne $value.policy_sha256) { throw 'TAG_AUTHORIZATION_POLICY_INVALID' }
    return [ordered]@{ reference = $reference; value = $value; policy = [ordered]@{ label = 'tag-push'; path = [string]$value.policy_path; sha256 = [string]$value.policy_sha256 } }
}
function Save-TagAuthorization([object]$policy,[object]$leaseEvidence) {
    if ($null -ne $state.mutation_authorization.tag) { $existing = Read-TagAuthorization $true; if ($existing.policy.path -ne $policy.path -or $existing.policy.sha256 -ne $policy.sha256 -or $existing.value.lease_sha256 -ne $leaseEvidence.lease_sha256) { throw 'TAG_AUTHORIZATION_IMMUTABLE_CONFLICT' }; return $existing }
    $authorizedAt = [DateTime]::UtcNow; $authFrom = [DateTime]::Parse([string]$leaseEvidence.valid_from).ToUniversalTime(); $authUntil = [DateTime]::Parse([string]$leaseEvidence.valid_until).ToUniversalTime(); if ($authorizedAt -lt $authFrom -or $authorizedAt -ge $authUntil) { throw 'TAG_AUTHORIZATION_OUTSIDE_LEASE' }
    $directory = Join-Path $evidence 'authorizations'; if (-not (Test-Path -LiteralPath $directory -PathType Container)) { New-Item -ItemType Directory -Path $directory -ErrorAction Stop | Out-Null }; $path = Join-Path $directory ("tag-v8.0.0-beta.1-$($policy.sha256)-$($leaseEvidence.lease_sha256).json")
    ...
    $json = $value | ConvertTo-Json -Depth 30; if (Test-Path -LiteralPath $path -PathType Leaf) { if ((Get-Content -Raw -LiteralPath $path) -ne $json) { throw 'TAG_AUTHORIZATION_CONFLICT' } } else { [IO.File]::WriteAllText($path,$json,[Text.UTF8Encoding]::new($false)) }
    $reference = [ordered]@{ effect_key = 'tag:v8.0.0-beta.1'; authorization_path = $path.Replace('\','/'); authorization_sha256 = (Get-FileHash -LiteralPath $path -Algorithm SHA256).Hash.ToLowerInvariant() }; $state.mutation_authorization.tag = $reference; Save-State $state
    $saved = Get-Content -Raw -LiteralPath $statePath | ConvertFrom-Json; if ($saved.mutation_authorization.tag.authorization_sha256 -ne $reference.authorization_sha256) { throw 'TAG_AUTHORIZATION_STATE_READBACK_INVALID' }
    return Read-TagAuthorization $true
}
function Read-PublicationPolicy([string]$label) {
    $null = Assert-PublicationLease
    $dir = Join-Path $evidence ('policy-before-' + $label + '-' + [guid]::NewGuid().ToString('N')); New-Item -ItemType Directory -Path $dir -ErrorAction Stop | Out-Null
    ...
    foreach ($item in @(@('actions.json',$actions),@('workflows.json',$workflows),@('ruleset.json',$ruleset),@('repository.json',$repoRaw))) { [IO.File]::WriteAllText((Join-Path $dir $item[0]),($item[1] -join [Environment]::NewLine),[Text.UTF8Encoding]::new($false)) }
    $a = ($actions -join [Environment]::NewLine) | ConvertFrom-Json; $w = ($workflows -join [Environment]::NewLine) | ConvertFrom-Json; $r = ($ruleset -join [Environment]::NewLine) | ConvertFrom-Json; $repoObject = ($repoRaw -join [Environment]::NewLine) | ConvertFrom-Json
    if ($repoObject.full_name -ne $repo -or $repoObject.default_branch -ne 'main' -or $a.enabled -ne $false -or $w.total_count -ne 0 -or $r.id -ne 20160628 -or $r.enforcement -ne 'active' -or $r.source -ne $repo -or @($r.bypass_actors).Count -ne 0 -or @($r.rules | Where-Object type -eq 'required_status_checks').Count -ne 0) { throw 'PUBLICATION_POLICY_INVALID' }
    $types = @($r.rules | ForEach-Object type); if (@(Compare-Object (@('deletion','non_fast_forward','pull_request','required_linear_history') | Sort-Object) ($types | Sort-Object)).Count -ne 0) { throw 'PUBLICATION_RULESET_INVALID' }; $pullRule = @($r.rules | Where-Object type -eq 'pull_request')[0]; $allowed = @(); if ($null -ne $pullRule -and $null -ne $pullRule.parameters -and $null -ne $pullRule.parameters.allowed_merge_methods) { $allowed = @($pullRule.parameters.allowed_merge_methods) }; if ($allowed -notcontains 'squash') { throw 'PUBLICATION_SQUASH_POLICY_INVALID' }; $includes = @(); if ($null -ne $r.conditions -and $null -ne $r.conditions.ref_name -and $null -ne $r.conditions.ref_name.include) { $includes = @($r.conditions.ref_name.include) }; if (-not ($includes -contains '~DEFAULT_BRANCH' -or $includes -contains 'refs/heads/main' -or $includes -contains 'main')) { throw 'PUBLICATION_RULESET_DEFAULT_BRANCH_INVALID' }
    return New-PolicyReceipt $label $dir $a $w $r $repoObject
}
...
$initialTagPresent = [bool]$state.publication.initial_tag_present; $tagAuthorization = $null; if ($direct.Count -gt 0 -and -not $initialTagPresent) { if ($null -eq $state.mutation_policy.tag) { throw 'TAG_EXISTING_EFFECT_POLICY_MISSING' }; $tagAuthorization = Read-TagAuthorization $true; if ($tagAuthorization.policy.path -ne $state.mutation_policy.tag.path -or $tagAuthorization.policy.sha256 -ne $state.mutation_policy.tag.sha256) { throw 'TAG_EXISTING_EFFECT_POLICY_AUTHORIZATION_MISMATCH' }; $policyReceipt = $tagAuthorization.policy } elseif ($direct.Count -gt 0 -and $initialTagPresent -and ($null -ne $state.mutation_policy.tag -or $null -ne $state.mutation_authorization.tag)) { throw 'PROVEN_READ_ONLY_TAG_HAS_MUTATION_EVIDENCE' }
if ($direct.Count -eq 0) {
    if ($initialTagPresent) { throw 'INITIAL_TAG_DISAPPEARED' }
    $policyReceipt = Read-PublicationPolicy 'tag-push'; if ($null -ne $state.mutation_policy.tag -and ($state.mutation_policy.tag.path -ne $policyReceipt.path -or $state.mutation_policy.tag.sha256 -ne $policyReceipt.sha256)) { throw 'TAG_POLICY_RESUME_CONFLICT' }; $state.mutation_policy.tag = $policyReceipt; Save-State $state
    $policyState = Get-Content -Raw -LiteralPath $statePath | ConvertFrom-Json; if ($policyState.mutation_policy.tag.sha256 -ne $policyReceipt.sha256) { throw 'TAG_POLICY_STATE_READBACK_INVALID' }
    $leaseEvidence = Assert-PublicationLease; $tagAuthorization = Save-TagAuthorization $policyReceipt $leaseEvidence
    ...
    if ($remoteMap['refs/heads/main'] -ne $mergedSha -or $remoteMap['refs/heads/codex/gwo-v8-beta1'] -ne $state.identities.beta1.sha -or $remoteMap['refs/heads/codex/gwo-v8-ga-plan'] -ne $state.identities.protected_ga.sha) { throw 'REMOTE_IDENTITY_CHANGED_BEFORE_TAG' }
    git -C $root fetch --no-tags origin refs/heads/main; $exit = $LASTEXITCODE; if ($exit -ne 0) { throw 'MAIN_FETCH_BEFORE_TAG_FAILED' }; $fetched = (git -C $root rev-parse FETCH_HEAD).Trim(); $exit = $LASTEXITCODE; if ($exit -ne 0 -or $fetched -ne $mergedSha) { throw 'MAIN_FETCH_BEFORE_TAG_INVALID' }
    $localType = @(git -C $root cat-file -t "refs/tags/$tagName" 2>&1); $localExit = $LASTEXITCODE
    if ($localExit -eq 0) { $localTarget = (git -C $root rev-parse "refs/tags/$tagName^{}").Trim(); $exit = $LASTEXITCODE; if ($exit -ne 0 -or $localType[0] -ne 'tag' -or $localTarget -ne $mergedSha) { throw 'LOCAL_TAG_CONFLICT' } } elseif ($localExit -eq 128) { git -C $root tag -a $tagName $mergedSha -m 'GWO V8 Beta1 - Core Preview'; $exit = $LASTEXITCODE; if ($exit -ne 0) { throw 'TAG_CREATE_FAILED' } } else { throw 'LOCAL_TAG_PROBE_FAILED' }
    $directImmediate = @(git -C $root ls-remote --tags origin "refs/tags/$tagName"); $exit = $LASTEXITCODE; if ($exit -ne 0 -or $directImmediate.Count -ne 0) { throw 'TAG_PREMUTATION_CONFLICT' }
    $finalTagLease = Assert-PublicationLease; $finalTagApprovalHash = (Get-FileHash -LiteralPath $approvalPath -Algorithm SHA256).Hash.ToLowerInvariant(); $finalTagLeaseHash = (Get-FileHash -LiteralPath $leasePath -Algorithm SHA256).Hash.ToLowerInvariant(); $finalTagNow = [DateTime]::UtcNow; $finalTagFrom = [DateTime]::Parse([string]$finalTagLease.valid_from).ToUniversalTime(); $finalTagUntil = [DateTime]::Parse([string]$finalTagLease.valid_until).ToUniversalTime(); if ($finalTagApprovalHash -ne $approvalHash -or $finalTagLeaseHash -ne $leaseHash -or $finalTagNow -lt $finalTagFrom -or $finalTagNow -ge $finalTagUntil) { throw 'TAG_FINAL_IMMEDIATE_LEASE_INVALID' }
    git -C $root push origin "refs/tags/$tagName"; $exit = $LASTEXITCODE; if ($exit -ne 0) { throw 'TAG_PUSH_FAILED' }
}
...
function Read-ReleaseAuthorization([bool]$required) {
    $reference = $state.mutation_authorization.release; if ($null -eq $reference) { if ($required) { throw 'RELEASE_EFFECT_WITHOUT_AUTHORIZATION' }; return $null }
...
function Save-ReleaseAuthorization([object]$policy,[object]$leaseEvidence) {
    ...
    $directory = Join-Path $evidence 'authorizations'; if (-not (Test-Path -LiteralPath $directory -PathType Container)) { New-Item -ItemType Directory -Path $directory -ErrorAction Stop | Out-Null }; $path = Join-Path $directory ("release-v8.0.0-beta.1-$($policy.sha256).json")
    ...
    $reference = [ordered]@{ effect_key = 'release:v8.0.0-beta.1'; authorization_path = $path.Replace('\','/'); authorization_sha256 = (Get-FileHash -LiteralPath $path -Algorithm SHA256).Hash.ToLowerInvariant() }; $state.mutation_authorization.release = $reference; Save-State $state
...
$releaseWriteRequired = -not [bool]$state.publication.initial_release_present; $releaseAuthorization = $null
if ($probeExit -ne 0) {
...
} elseif ($releaseWriteRequired) { $releaseAuthorization = Read-ReleaseAuthorization $true; $policyReceipt = $releaseAuthorization.policy
} elseif ($null -ne (Read-ReleaseAuthorization $false)) { throw 'PROVEN_READ_ONLY_RELEASE_HAS_AUTHORIZATION' }
...
function Assert-PolicyReceipt([object]$receipt) {
    ...
    $directory = Split-Path -LiteralPath $receipt.path -Parent; $fileValues = [ordered]@{}
