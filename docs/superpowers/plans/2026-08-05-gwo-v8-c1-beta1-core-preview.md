...
- immutable per-mutation policy and pre-mutation authorization receipt paths,
  SHA-256 values, lease windows, and `authorized_at` timestamps;
...
$state = [ordered]@{
    ...
    mutation_authorization = [ordered]@{ pr_merge = $null; tracker = @(); release = $null }
    pr = [ordered]@{ created_by_plan = $null; ready_at = $null; ready_approval_sha256 = $null; ready_lease_sha256 = $null; merge = $null }
    tracker = [ordered]@{}
...
$readyClock = [DateTime]::UtcNow; $leaseFrom = [DateTime]::Parse([string]$lease.valid_from).ToUniversalTime(); $leaseUntil = [DateTime]::Parse([string]$lease.valid_until).ToUniversalTime()
if ($lease.schema -ne 'gwo-v8-c1-integration-lease.v1' -or $lease.state -ne 'active' -or $lease.id -ne $approval.integration_lease_id -or $lease.owner -ne $approval.owner -or $lease.repository -ne $repo -or $lease.scope -ne 'repository-global-remote-writer' -or $lease.exclusive_remote_writer_control -ne $true -or $lease.base_sha -ne $state.identities.base.sha -or $lease.base_tree -ne $state.identities.base.tree -or $lease.head_sha -ne $state.identities.beta1.sha -or $lease.head_tree -ne $state.identities.beta1.tree -or @(Compare-Object $expectedPrScope $leaseScope).Count -ne 0 -or $lease.approval_sha256 -ne $approvalHash -or $readyClock -lt $leaseFrom -or $readyClock -ge $leaseUntil) { throw 'PR_LEASE_RELOAD_INVALID' }
...
$repoPolicyRaw = @(gh api repos/$repo 2>&1); $exit = $LASTEXITCODE; if ($exit -ne 0) { throw 'REPOSITORY_POLICY_READ_FAILED' }; $repoPolicy = ($repoPolicyRaw -join [Environment]::NewLine) | ConvertFrom-Json; if ($repoPolicy.full_name -ne $repo -or $repoPolicy.default_branch -ne 'main') { throw 'DEFAULT_BRANCH_POLICY_INVALID' }; [IO.File]::WriteAllText((Join-Path $policyDir 'repository.json'),($repoPolicyRaw -join [Environment]::NewLine),[Text.UTF8Encoding]::new($false))
$policyReceipt = New-PolicyReceipt 'pr-ready' $policyDir $a $w $r $repoPolicy; $didReady = $pr.draft -eq $true
if ($didReady) { $state.mutation_policy.pr_ready = $policyReceipt; Save-State $state; $policyState = Get-Content -Raw -LiteralPath $statePath | ConvertFrom-Json; if ($policyState.mutation_policy.pr_ready.sha256 -ne $policyReceipt.sha256) { throw 'PR_READY_POLICY_STATE_READBACK_INVALID' }; $approvalImmediate = Get-Content -Raw -LiteralPath $approvalPath | ConvertFrom-Json; $leaseImmediate = Get-Content -Raw -LiteralPath $leasePath | ConvertFrom-Json; $approvalHashImmediate = (Get-FileHash -LiteralPath $approvalPath -Algorithm SHA256).Hash.ToLowerInvariant(); $leaseHashImmediate = (Get-FileHash -LiteralPath $leasePath -Algorithm SHA256).Hash.ToLowerInvariant(); $fromImmediate = [DateTime]::Parse([string]$leaseImmediate.valid_from).ToUniversalTime(); $untilImmediate = [DateTime]::Parse([string]$leaseImmediate.valid_until).ToUniversalTime(); if ($approvalHashImmediate -ne $approvalHash -or $leaseHashImmediate -ne $leaseHash -or $leaseImmediate.id -ne $approvalImmediate.integration_lease_id -or $leaseImmediate.owner -ne $approvalImmediate.owner -or $leaseImmediate.scope -ne 'repository-global-remote-writer' -or $leaseImmediate.exclusive_remote_writer_control -ne $true -or $leaseImmediate.base_sha -ne $state.identities.base.sha -or $leaseImmediate.head_sha -ne $state.identities.beta1.sha -or [DateTime]::UtcNow -lt $fromImmediate -or [DateTime]::UtcNow -ge $untilImmediate) { throw 'PR_READY_LEASE_FINAL_RELOAD_INVALID' }; gh pr ready $prNumber --repo $repo; $exit = $LASTEXITCODE; if ($exit -ne 0) { throw 'PR_READY_FAILED' } }
...
if ($didReady) { $state.pr.ready_at = [DateTime]::UtcNow.ToString('o'); $state.mutation_policy.pr_ready = $policyReceipt }; $state.pr.ready_approval_sha256 = $approvalHash; $state.pr.ready_lease_sha256 = $leaseHash; Save-State $state; $check = Get-Content -Raw -LiteralPath $statePath | ConvertFrom-Json; if ($check.pr.ready_lease_sha256 -ne $leaseHash) { throw 'PR_READY_STATE_READBACK_INVALID' }
~~~
...
foreach ($title in $map.Keys) {
    ...
    if ($effectMissing) {
        ...
        $leaseEvidence = Assert-TrackerLease; $authorization = Save-TrackerAuthorization $effectKey $policyReceipt $leaseEvidence
...
if ($reopenRequired -and $issue137.semantic.state -eq 'closed') {
    ...
    $leaseEvidence = Assert-TrackerLease; $reopenAuthorization = Save-TrackerAuthorization $reopenEffectKey $policyReceipt $leaseEvidence
...
if ($direct.Count -eq 0) {
    $policyReceipt = Read-PublicationPolicy 'tag-push'; $state.mutation_policy.tag = $policyReceipt; Save-State $state
...
if ($probeExit -ne 0) {
    ...
    $leaseEvidence = Assert-PublicationLease; $releaseAuthorization = Save-ReleaseAuthorization $policyReceipt $leaseEvidence
...
old windows. Validate every merge/tracker/Release `authorized_at` against its
own original window. `pr_ready` and `pr_merge` policy receipts are MANDATORY;
`pr_create` follows `created_by_plan`, tracker receipts follow the frozen
before-snapshot deltas/effects, and tag/Release receipts follow the frozen
...
function Assert-MutationAuthorization([string]$gate,[string]$effectKey,[string]$operation,[object]$reference) {
    ...
    if ($gate -eq 'pr' -and ($value.base_sha -ne $state.identities.base.sha -or $value.head_sha -ne $state.identities.beta1.sha -or [int]$value.pr_number -ne [int]$state.pr.number)) { throw 'MERGE_AUTHORIZATION_IDENTITY_INVALID' }
...
$policyReceiptDigests['pr_ready'] = Assert-PolicyReceipt $state.mutation_policy.pr_ready
...
if ($state.pr.created_by_plan -eq $true) { if ($null -eq $state.mutation_policy.pr_create) { throw 'PR_CREATE_POLICY_MISSING' }; $policyReceiptDigests['pr_create'] = Assert-PolicyReceipt $state.mutation_policy.pr_create
} elseif ($state.pr.created_by_plan -eq $false -and $null -eq $state.mutation_policy.pr_create) { $policyReceiptDigests['pr_create'] = $null # PROVEN_READ_ONLY PR reuse
...
if (-not [bool]$state.publication.initial_tag_present) { if ($null -eq $state.mutation_policy.tag) { throw 'TAG_POLICY_REQUIRED_BY_INITIAL_STATE' }; $policyReceiptDigests['tag'] = Assert-PolicyReceipt $state.mutation_policy.tag
} elseif ($null -eq $state.mutation_policy.tag) { $policyReceiptDigests['tag'] = $null # PROVEN_READ_ONLY tag
} else { throw 'PROVEN_READ_ONLY_TAG_HAS_POLICY' }
