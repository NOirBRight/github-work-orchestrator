...
function Assert-MergePolicyAttempt([object]$receipt) {
    try {
        ...
        $directory = Split-Path -LiteralPath $receiptPath -Parent; $values = [ordered]@{}
...
$alreadyMerged = $prBefore.merged -eq $true; if ($alreadyMerged) { $orphanDirectory = Join-Path $evidence 'authorizations'; $orphanPaths = @(); if (Test-Path -LiteralPath $orphanDirectory -PathType Container) { $orphanPaths = @(Get-ChildItem -LiteralPath $orphanDirectory -Filter 'pr-merge-*.json' -File | Sort-Object FullName | Where-Object { $null -eq $state.mutation_authorization.pr_merge -or $_.FullName.Replace([char]92,'/') -ne $state.mutation_authorization.pr_merge.authorization_path }) }; if ($orphanPaths.Count -gt 0) { throw 'MERGE_AUTHORIZATION_ORPHAN_CONFLICT' } }; $mergeAuthorization = Read-MergeAuthorization; $policyReceipt = $state.mutation_policy.pr_merge
...
function Assert-TrackerPolicyAttempt([object]$receipt,[string]$effectKey) {
    try {
        if ($null -eq $receipt -or [string]::IsNullOrWhiteSpace([string]$effectKey) -or $receipt.effect_key -ne $effectKey -or [string]::IsNullOrWhiteSpace([string]$receipt.label) -or ([string]$receipt.label) -notmatch '^tracker-' -or [string]::IsNullOrWhiteSpace([string]$receipt.path) -or [string]::IsNullOrWhiteSpace([string]$receipt.sha256)) { throw "TRACKER_POLICY_ATTEMPT_INVALID:$effectKey" }
        $receiptPath = ([IO.Path]::GetFullPath([string]$receipt.path)).Replace([char]92,'/')
