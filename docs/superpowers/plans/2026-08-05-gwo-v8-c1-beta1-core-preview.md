...
- immutable per-mutation policy receipt paths and SHA-256 values;
...
non-expired window, `scope=repository-global-remote-writer`,
...
$alreadyMerged = $prBefore.merged -eq $true; $policyReceipt = $state.mutation_policy.pr_merge
if (-not $alreadyMerged) {
    ...
    if ($null -eq $policyReceipt) { $policyCandidates = @(Get-ChildItem -LiteralPath $evidence -Directory -Filter 'policy-before-merge-*' | ForEach-Object { Join-Path $_.FullName 'receipt.json' } | Where-Object { Test-Path -LiteralPath $_ -PathType Leaf }); if ($policyCandidates.Count -ne 1) { throw 'MERGE_POLICY_RECEIPT_RESUME_INVALID' }; $policyReceipt = [ordered]@{ label = 'pr-merge'; path = $policyCandidates[0].Replace('\','/'); sha256 = (Get-FileHash -LiteralPath $policyCandidates[0] -Algorithm SHA256).Hash.ToLowerInvariant() } }
}
...
function Read-Policy([string]$label) {
    Assert-TrackerLease
    ...
    $receipt = New-PolicyReceipt ('tracker-' + $label) $dir $a $w $r $repoObject
    $state.mutation_policy.tracker = @($state.mutation_policy.tracker) + @($receipt); Save-State $state
    $saved = Get-Content -Raw -LiteralPath $statePath | ConvertFrom-Json; $savedReceipts = @($saved.mutation_policy.tracker); if ($savedReceipts.Count -eq 0 -or $savedReceipts[-1].sha256 -ne $receipt.sha256) { throw 'TRACKER_POLICY_STATE_READBACK_INVALID' }
    return $receipt
}
function Optional-Collection([string]$endpoint,[string]$name) { $raw = @(gh api $endpoint 2>&1); $code = $LASTEXITCODE; if ($code -eq 0) { try { return @(($raw -join [Environment]::NewLine) | ConvertFrom-Json) } catch { throw "${name}_MALFORMED" } }; if (($raw -join [Environment]::NewLine) -match '404') { return @() }; throw "${name}_READ_FAILED" }
...
foreach ($title in $map.Keys) {
    $found = @($milestones | Where-Object title -eq $title); if ($found.Count -gt 1) { throw "MILESTONE_CONFLICT:$title" }
    if ($found.Count -eq 0) {
        $policyReceipt = Read-Policy ('create-' + ($title -replace '\s+','-'))
    ...
    $effect = [ordered]@{ schema = 'gwo-v8-c1-tracker-effect.v2'; kind = 'milestone'; repository = $repo; merged_main_sha = $state.pr.merge.merge_sha; milestone = Milestone-Effect $finalMilestone[0] }
...
foreach ($title in $map.Keys) {
    ...
    foreach ($number in $map[$title]) {
        ...
        if ($null -eq $before.semantic.milestone) {
            $policyReceipt = Read-Policy ('issue-' + $number)
        ...
        $effect = [ordered]@{ schema = 'gwo-v8-c1-tracker-effect.v2'; kind = 'issue-milestone'; repository = $repo; merged_main_sha = $state.pr.merge.merge_sha; number = [int]$number; issue_url = [string]$final.semantic.html_url; milestone = [ordered]@{ id = [long]$milestone.id; number = $milestoneNumber; title = $title } }
        $effects += Save-Effect ('issue-' + $number) $effect
    }
}
$issue137 = Read-Issue 137; $issue114 = Read-Issue 114; $issue115 = Read-Issue 115; $reopenAllowed = $expectedMutationSet -contains 'reopen:#137=when_closed_and_#114_or_#115_open'
if ($issue137.semantic.state -eq 'closed' -and ($issue114.semantic.state -eq 'open' -or $issue115.semantic.state -eq 'open')) {
    if (-not $reopenAllowed) { throw 'REOPEN_NOT_APPROVED' }
    $policyReceipt = Read-Policy 'reopen-137'
...
} elseif ($issue137.semantic.state -eq 'open') { $after137 = $issue137 }
if ($null -ne $after137 -and $after137.semantic.state -eq 'open') { $effects += Save-Effect 'issue-137-reopen' ([ordered]@{ schema = 'gwo-v8-c1-tracker-effect.v2'; kind = 'conditional-reopen'; repository = $repo; merged_main_sha = $state.pr.merge.merge_sha; number = 137; issue_url = [string]$after137.semantic.html_url; condition = '#137 closed and (#114 open or #115 open)'; final_state = 'open' }) }
...
$policyReceiptDigests = [ordered]@{}; foreach ($slot in @('pr_create','pr_ready','pr_merge','tag','release')) { $entry = $state.mutation_policy.$slot; if ($null -eq $entry) { $policyReceiptDigests[$slot] = $null } else { $policyReceiptDigests[$slot] = Assert-PolicyReceipt $entry } }; $trackerPolicyDigests = @(); foreach ($entry in @($state.mutation_policy.tracker)) { $trackerPolicyDigests += Assert-PolicyReceipt $entry }; $policyReceiptDigests['tracker'] = $trackerPolicyDigests
...
foreach ($gate in @('pr','tracker','publication')) {
    ...
    if ($leaseFrom -ge $leaseUntil -or $savedFrom -ne $leaseFrom -or $savedUntil -ne $leaseUntil -or [DateTime]::UtcNow -lt $leaseFrom -or [DateTime]::UtcNow -ge $leaseUntil) { throw "CLOSURE_LEASE_WINDOW_INVALID:$gate" }
