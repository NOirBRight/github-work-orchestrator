# GWO V8 Cutover Guard Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Deliver Issue #118 as the Beta3 cutover-candidate gate: a read-only, fail-closed Guard proves the V6.1-to-V8 prerequisites and returns a digest-bound activation token, while the existing writer-generation/Activation Receipt protocol remains the only authority-transfer commit point.

**Architecture:** Add one host/release-control adapter module, `cutover_guard.py` (not a sixth domain deep module), whose ports expose reads only and whose evaluator never calls a repository, SQLite, GitHub, process, Runtime, or package-install writer. It returns a complete human-readable go/no-go report and an immutable preflight token; a guarded adapter at the existing `WriterCutoverController` boundary re-reads the same facts immediately before invoking the existing fenced activation. The Guard does not run the root Canary, enable V8 by default, create a Plan Revision, or implement rollback; those remain the #119 GA and existing compensating-transition responsibilities.
...
- No task changes the dirty primary checkout at `D:\Workstation\github-work-orchestrator`; execute each task in a fresh clean isolated worktree from the exact prerequisite `origin/main` SHA.
...
def _decode_bundle(value: object) -> CutoverReadbackBundle:
    ...
    subject_data = dict(value["subject"])
    for name in (
    ...
    runtime_data = dict(readbacks["runtime"])
    runtime_data["selectors"] = tuple(
        RuntimeSelectorReadback(**item) for item in runtime_data["selectors"]
    )
    package_data = dict(readbacks["packages"])
    package_data["source_packages"] = tuple(
        PackageIdentity(**item) for item in package_data["source_packages"]
    )
    package_data["installed_packages"] = tuple(
        PackageIdentity(**item) for item in package_data["installed_packages"]
    )
    bundle = CutoverReadbackBundle(
        schema=value["schema"],
        subject=subject,
...
1. Task 1 defines the immutable contract and pure evaluator.
2. Task 2 adds the AST production-path proof and package/install readback; it can be reviewed independently after Task 1.
3. After Task 2, run Task 3 and then Task 4 serially. Although their primary production modules differ, both regenerate `skills/orchestrator/.skill-package.json`; one manifest owner is not enough when the tasks also have independent commits, so the plan forbids parallel execution here.
4. Task 5 (root API/predecessor reachability) follows Tasks 2–4 because its tests inspect the final host import graph and token-enforced writer path.
5. Task 6 (CLI) follows Task 3 and consumes only the report/readback-bundle interface; Task 7 is serial final verification.

Do not use more than five subagents if execution is delegated. A safe delegation is one fresh gpt-5.6-luna subagent with max reasoning per independent task, but no subagent is needed to write this plan.
...
class CutoverGuard:
    ...
    def evaluate(self, subject: CutoverSubject) -> CutoverGuardReport:
        ...
        def read(name: str, operation: Callable[[], object]) -> object | None:
            try:
                value = operation()
        ...
        readback_digest = digest_value(
            {name: readbacks[name] for name in ordered_names}
        )
...
The temporary manifests are overwritten by the validator fixture only when a test explicitly needs a matching digest; the helper never calls `write_manifest`, `install_atomic`, `shutil.copytree`, or `os.replace`.
...
and readback methods remain in the class alongside these methods.
...
~~~powershell
py -3.13 -m pytest tests/test_v8_cutover_guard_host.py tests/test_v8_successor_host.py tests/test_v8_plancontrol_production.py -q
~~~
...
py -3.13 -c "from pathlib import Path; import json; from scripts.sync_orchestrator import expected_manifest; package=Path('skills/orchestrator'); assert json.loads((package/'.skill-package.json').read_text(encoding='utf-8')) == expected_manifest(package); print('manifest OK')"
git diff --check
git add skills/orchestrator/scripts/gwo_v8/cutover_guard.py skills/orchestrator/scripts/gwo_v8/plan_control_host.py skills/orchestrator/.skill-package.json tests/test_v8_cutover_guard_host.py
...
**Interfaces:**
- Consumes: CutoverGuard, CutoverSubject, CutoverGuardReceipt, and ProductionCutoverGuardHost.validate_activation from Tasks 1 and 3.
...
def test_stale_guard_token_returns_blocked_before_v61_stop_or_activation(tmp_path):
    fixture = activation_fixture(tmp_path)
    report = fixture.guard.evaluate(fixture.subject)
    assert report.receipt is not None
...
def activation_fixture(
    ...
    *,
    ...
    guard = CutoverGuard(harness.sources)
...
The canary is passed only to `WriterCutoverController.cutover`; `CutoverGuard.evaluate` receives only the subject. Every fixture target is beneath pytest `tmp_path`, and no fixture calls the public default host, changes the default writer, installs a package, creates a release tag, or claims GA admission.
...
class WriterCutoverController:
    ...
    def cutover(
        ...
        *,
        ...
        try:
            self.guard.validate_activation_token(guard_subject, guard_receipt)
...
The code above runs the four Guard checks before any transition read that could
...
Verify that WriterCutoverController.cutover has exactly one call site for validate_activation_token, that it occurs before legacy.stop, transitions.publish, or publication.publish_and_activate, and that rollback never deletes an Activation Receipt. Run:
...
def test_implement_skill_has_no_predecessor_execution_route():
    ...
    for forbidden in (
        ...
        "GoalDriver",
        "Matt /implement remains a separate single-ticket workflow",
    ):
...
The preceding production-composition Result must be merged and read back before Step 2. Only then does `real_static_subject(ROOT, entry_refs)` compute the actual digest of the audited files; `static_subject` with `b * 64` is forbidden for either real-root test. The import-only edits move every removed root import to its owning deep module. Use this exact mapping: predecessor Kernel, GoalDriver, and ImplementGwo* imports go to their historical direct modules only in tests that still exercise compatibility fixtures; RuntimeGateway/RuntimeConfiguration/RuntimeProfile imports go to gwo_v8.runtime_gateway/gwo_v8.runtime_profile; PlanCompiler goes to gwo_v8.compiler; CandidateGate, PlanControl, ExecutionKernel, IntegrationBatch, and successor types go to their named modules. Do not add a __getattr__ compatibility fallback to __init__.py.
...
~~~powershell
py -3.13 -m pytest tests/test_v8_cutover_guard_api_boundary.py tests/test_orchestrator_package.py tests/test_v8_runtime_gateway.py tests/test_v8_candidate_gate.py tests/test_v8_successor_host.py -q
~~~
...
~~~powershell
$beta2Sha = (git rev-parse v8.0.0-beta.2).Trim()
...
~~~powershell
$plan = "docs/superpowers/plans/2026-08-03-gwo-v8-cutover-guard.md"
$patterns = @(
    (@("T", "B", "D") -join ""),
    (@("T", "O", "D", "O") -join ""),
    (@("as", "appropriate") -join " "),
