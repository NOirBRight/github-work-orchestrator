...
component, not an Agent, Coordinator turn, PlanSpec work entry, or semantic
authority.
...
the time arrives. This timer path recovers lost callbacks, drives
interactive-wait grace expiry, and performs targeted readback without sampling
an LLM. On Watchdog restart, active Campaign state reconstructs the
outstanding event subscriptions and timers.
...
`Decision` or separately accepted bounded stale/semantic trigger can wake lazy
Coordination. Healthy Campaigns therefore spend no LLM tokens on supervision.
