...
After the edit loop's latest cheap affected checks pass and their observed tree
digest matches the immutable Candidate, the current Work Attempt parent invokes
`code-review`; it does not rerun those checks merely to enter Review. The
parent launches the required read-only Internal Subagents in parallel with no
parent transcript: each receives the base and Candidate SHAs, diff command,
commit list, canonical Spec and standards sources, and relevant Check Manifest.
...
The parent requests those children through the Runtime Adapter, so the selected
Paseo child may use a different provider and model from its parent.
Review children consume no separate GWO capacity slot; their fixed fan-out is
bounded to two standard axes plus at most one strict specialist. The parent
retains its Worker Active Turn Slot until those children finish or enter a
...
stored independently as soon as it finishes. The Work Attempt parent only
...
typed axis observation itself. A parent never converts free-form prose into
...
Local axis records are persisted before publication. After the one eligible
push, GWO publishes their compact durable manifest without rerunning Review.
This keeps independent review and exact-Candidate authority while removing an
entire scheduling, capacity, materialization, retry, and Result lifecycle.
